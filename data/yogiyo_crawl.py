...
def get_restaurant_infos(query, category):
    ...
    links_lst = lst
    infos_by_restaurant = {'restaurants': []}

    for i in range(links_no):
        infos = {}

        # 2. 가게 이름
        infos['name'] = restaurants[i]

        # 3. 가게 평균 평점: 평점 정보 없을 시 0.0으로 저장
        restaurant_rate_txt = running_driver.find_elements(By.CLASS_NAME, 'ico-star1')[i].get_attribute('innerHTML')
        infos['avg_rate'] = float(re.findall('[\d.]+', restaurant_rate_txt)[0])
        ...
        if promotion_info:
            restaurant_promo['promo_on_condition'] = price_to_int(promotion_info)
            
            on_condition_lst = re.findall('[\d,]+원 이상 주문 시', promotion_info)
            # '~ 이상 주문 시' 문구 있으면 조건 추가
            if on_condition_lst:
                on_condition = on_condition_lst[0]
                restaurant_promo['condition'] = on_condition
            else: # 해당 문구가 없으면 최소주문금액 이상 주문 시 할인인 것으로 확인됨 (요기요 앱 참고)
                restaurant_promo['condition'] = f'{infos["delivery_available_price"]}원 이상 주문 시'

        else:
            restaurant_promo['promotion_on_condition'] = ''
            restaurant_promo['on_condition'] = ''

        # 추가할인 - 추가할인 없으면 '0%'로 저장
        additional_promo = running_driver.find_element(By.XPATH, '//*[@id="content"]/div[2]/div[1]/div[1]/div[2]/ul/li[7]/span[2]').get_attribute('innerHTML')
        restaurant_promo['additional_dc'] = re.findall('[\d]+%', additional_promo)[0]

        infos['restaurant_promotions'] = restaurant_promo
        ...
        for menu, oos, desc, img, price, dc in zip(menus, out_of_stock, descs, imgs, prices, dcs):
            menus_info_dict[menu] = {'out_of_stock': oos, 'description': desc, 'image': img, 'price': price, 'price_dc': dc}
        ...
        infos_by_restaurant['restaurants'].append(infos)
